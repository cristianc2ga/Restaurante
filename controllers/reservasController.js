...
module.exports = {
  ...
  crear: function (req, res) {
    ...
    restaurante
      .retornarDatosId(req.params.idRestaurante)
      .then((registro) => {
        if (registro) {
          reserva
            .obtenerDatosIdRestaurante(req.params.idRestaurante)
            .then((registroRes) => {
              if (registroRes) {
                var nMesas = new Array(
                  1,
                  2,
                  3,
                  4,
                ...
                var resultArray = Object.values(
                  JSON.parse(JSON.stringify(registroRes))
                );
                ...
                var mesasDisponibles = nMesas.filter(
                ...
                return res
                  .status(500)
                  .send("No existen reserva en el restaurante");
              }
            })
            .catch((err) => {
              return res
                .status(500)
                .send("Error obteniendo la reservas para el restaurante");
            });
        } else {
        }
      })
  ...
  insertar: function (req, res) {
    ...
    if (req.body.personas > 4) {
      return res
        .status(500)
        .send(
          "Supera el número máximo de personas, intente con un valor menor o igual a 4"
        );
    }
    restaurante
      .retornarDatosId(req.body.idRestaurante)
      .then((registro) => {
        if (registro) {
          var nReservas = registro.reservas + 1;
          restaurante
            .actualizarReserva(req.body.idRestaurante, nReservas)
            .then(() => {
              reserva
                .insertar(req.body, userId)
            ...
            .catch((err) => {
              return res
                .status(500)
                .send("Error actualizando las reservas del restaurante");
            });
        } else {
  ...
  eliminar: function (req, res) {
    reserva
      .retornarDatosId(req.params.id)
      .then((registro) => {
        restaurante
          .retornarDatosId(registro.idrestaurante)
          .then((registroRes) => {
            if (registroRes) {
              var nReservas = registroRes.reservas - 1;
              restaurante
                .actualizarReserva(registroRes.id, nReservas)
                .then(() => {
                  reserva
                    ...
                    .catch((err) => {
                      return res
                        .status(500)
                        .send("Error eliminando la reserva");
                    });
                })
                .catch((err) => {
                  return res
              ...
              return res
                .status(500)
                .send("No existe el restaurante con ese id");
            }
          })
          .catch((err) => {
            return res.status(500).send("Error obteniendo el restaurante");
  ...
  editar: function (req, res) {
    ...
    reserva
      ...
      .then((registro) => {
        if (registro) {
          reserva
            .obtenerDatosIdRestaurante(registro.idRestaurante)
            .then((registroRes) => {
              if (registroRes) {
                var nMesas = new Array(
                  1,
          ...
          reserva
            ...
            .then((registroRes) => {
              if (registroRes) {
                ...
                var mesasDisponibles = nMesas.filter((element) =>
                  temp.includes(element)
                );

                let existeElemento = mesasDisponibles.includes(
                  parseInt(req.body.mesa)
                );
                console.log(existeElemento);
                if (
                  existeElemento &&
                  parseInt(req.body.mesa) == registro.mesa
                ) {
                  return res
                    .status(400)
                    .send(
                      "Esta mesa ya esta reservada regrese y seleccione una de las mesas disponibles"
                    );
                }
                reserva
                  .actualizar(req.body)
                  ...
                  .catch((err) => {
                    return res
                      .status(500)
                      .send("Error actualizando la reserva");
                  });
              } else {
                return res
                  .status(500)
                  .send("No existen reservas en el restaurante");
              }
            })
            .catch((err) => {
              return res
